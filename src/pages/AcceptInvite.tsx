...
export default function AcceptInvite() {
  ...
  const showForm = hasInviteHash && user;
  const showInvalid = !authLoading && !user && !hasInviteHash;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: t('auth.acceptInviteSuccessTitle'),
        description: t('auth.acceptInviteSuccessDesc'),
      });
      ...
      <Layout {...layoutProps}>
        <div className="flex flex-col gap-6">
          <h1 className="animate-element animate-delay-100 text-4xl md:text-5xl font-semibold leading-tight">
            <span className="font-light tracking-tighter">{t('auth.acceptInviteTitle')}</span>
          </h1>
          <p className="animate-element animate-delay-200 text-muted-foreground">
            {t('auth.acceptInviteSubtitle')}
          </p>

          {authLoading && hasInviteHash && (
          ...
          {showInvalid && (
            <div className="animate-element animate-delay-300 space-y-5">
              <p className="text-center text-sm text-muted-foreground">
                {t('auth.acceptInviteInvalidLinkDesc')}
              </p>
